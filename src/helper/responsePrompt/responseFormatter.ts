...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
    ...
    fileEditWithUserChanges: (
        ...
        (autoFormattingEdits
            ...
            : "") +
        `The updated content, which includes both your original modifications and the additional edits, has been successfully saved to ${relPath}. Here is the full, updated content of the file that was saved:\n\n` +
    ...
    clineRulesGlobalDirectoryInstructions: (globalClineRulesFilePath: string, content: string) =>
        `# .clinerules/\n\nThe following is provided by a global .clinerules/ directory, located at ${globalClineRulesFilePath}, where the user has specified instructions for all working directories:\n\n${content}`,
    ...
    fileContextWarning: (editedFiles: string[]): string => {
        ...
        return (
            ...
            `${editedFiles.map((file) => ` ${path.resolve(file)}`).join("\n")}\n` +
